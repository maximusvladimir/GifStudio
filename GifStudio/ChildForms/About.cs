...
namespace GifStudio.ChildForms
{
    public partial class About : Form
    {
        ...
        {
            InitializeComponent();
            string assemble = AssemblyInfo.Company + "\r\nVersion " + AssemblyInfo.Version + "\r\n" + AssemblyInfo.Copyright
                    + "\r\n\r\n" + AssemblyInfo.Description;
            label2.Text = assemble;
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            save.Filter = "Save to tar.gz (*.tar.gz)|(*.tar.gz)";//"Save to folder (*.afolder)|*.afolder|Save to tar.gz (*.tar.gz)|(*.tar.gz)";
            save.RestoreDirectory = true;
            save.SupportMultiDottedExtensions = true;
            save.Title = "Save as a tar.gz";
            save.ShowDialog(this);

            if (save.FileName != null)
            {
                if (System.IO.Path.GetExtension(save.FileName).ToLower() == ".afolder")
                {
                   // Gifbrary.Common.FFmpeg.UnloadSourceFolder(
                   //     save.FileName.Replace(System.IO.Path.GetFileName(save.FileName), ""));
                }
                else
                    Gifbrary.Common.FFmpeg.UnloadSource7z(save.FileName);
            }
        }
    }
    /// <summary>
